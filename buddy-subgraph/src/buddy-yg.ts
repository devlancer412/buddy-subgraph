...
} from '../generated/BuddyYG/Buddy';
import { Buddy, Diamond, DiamondSlot } from '../generated/schema';
import { BuddyAttributes, ygIPFSHash } from './constants';

function getOrCreateYGBuddy(id: BigInt): Buddy {
  let entity = Buddy.load('YG' + '-' + id.toString());

  if (!entity) {
    entity = new Buddy('YG' + '-' + id.toString());
    ...
    const path = ygIPFSHash + '/' + id.toString() + '.json';
      ...
      switch (
        ...
        case 'Class':
          ...
          switch (className) {
            case 'Athlete':
              entity.type = 'YGA';
              entity.name = 'YG Athlete #' + id.toString();
              break;
            case 'Champion':
              entity.type = 'YGC';
              entity.name = 'YG Champion #' + id.toString();
              break;
            default:
              entity.type = 'YGR';
              entity.name = 'YG Rookie #' + id.toString();
              break;
          }
          break;
        case 'Diamond Slot 1':
...
export function handleTransfer(event: TransferEvent): void {
  let buddy = getOrCreateYGBuddy(event.params.tokenId);
