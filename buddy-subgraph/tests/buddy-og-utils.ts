...
import {
  ...
  Transfer,
  UpdatedAttribute
} from "../generated/BuddyOG/BuddyOG"
...
export function createDiamondInjectEvent(
  slotId: BigInt,
  diamondId: BigInt,
  buddyId: BigInt
): DiamondInject {
  let diamondInjectEvent = changetype<DiamondInject>(newMockEvent())

  diamondInjectEvent.parameters = new Array()

  diamondInjectEvent.parameters.push(
    new ethereum.EventParam("slotId", ethereum.Value.fromUnsignedBigInt(slotId))
  )
  diamondInjectEvent.parameters.push(
    new ethereum.EventParam(
      "diamondId",
      ethereum.Value.fromUnsignedBigInt(diamondId)
    )
  )
  diamondInjectEvent.parameters.push(
    new ethereum.EventParam(
      "buddyId",
      ethereum.Value.fromUnsignedBigInt(buddyId)
    )
  )

  return diamondInjectEvent
}

export function createDiamondRejectEvent(
  slotId: BigInt,
  diamondId: BigInt,
  buddyId: BigInt
): DiamondReject {
  let diamondRejectEvent = changetype<DiamondReject>(newMockEvent())

  diamondRejectEvent.parameters = new Array()

  diamondRejectEvent.parameters.push(
    new ethereum.EventParam("slotId", ethereum.Value.fromUnsignedBigInt(slotId))
  )
  diamondRejectEvent.parameters.push(
    new ethereum.EventParam(
...
export function createUpdatedAttributeEvent(
  tokenId: BigInt,
  attribute: i32,
  value: BigInt
): UpdatedAttribute {
  let updatedAttributeEvent = changetype<UpdatedAttribute>(newMockEvent())

  updatedAttributeEvent.parameters = new Array()

  updatedAttributeEvent.parameters.push(
    new ethereum.EventParam(
      "tokenId",
      ethereum.Value.fromUnsignedBigInt(tokenId)
    )
  ...
  updatedAttributeEvent.parameters.push(
    new ethereum.EventParam(
      "attribute",
      ethereum.Value.fromUnsignedBigInt(BigInt.fromI32(attribute))
    )
  )
  updatedAttributeEvent.parameters.push(
